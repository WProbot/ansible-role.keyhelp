...
#=======================================================================================================================
    ...
    function check_supported_os()
    {
        ...
        #
        #if [ "$os" = 'Ubuntu_20' ]; then
        #    echo
        #    warn "The support for Ubuntu 20 is currently BETA state. It should only be used for testing purpose."
...
#=======================================================================================================================
    ...
    # Checks if script is running on a a 64-bit system.
    #
    ...
    #
    # return  string  Main version number (eg 5.5, 7.0)
...
#=======================================================================================================================
    ...
    function install_php()
    {
        ...
        case `get_os` in
            "Ubuntu_14.04"|"Debian_8")
                apt-get install -y -qq php5 php5-mysqlnd php5-intl php5-readline
                ;;
    ...
    function install_ioncube()
    {
        ...
        dpkg --compare-versions `get_php_version` "ge" "7.0"
        if [ $? -eq "0" ]; then
            local ioncubeIniPath="/etc/php/`get_php_version`/mods-available/ioncube.ini"
        else
            local ioncubeIniPath="/etc/php5/mods-available/ioncube.ini"
        fi

        if [ `is_64bit` = 'true' ]; then
            local downloadUrl="http://downloads3.ioncube.com/loader_downloads/ioncube_loaders_lin_x86-64.tar.gz"
        else
            local downloadUrl="http://downloads3.ioncube.com/loader_downloads/ioncube_loaders_lin_x86.tar.gz"
        fi

        # Download
        wget --prefer-family="$PREFERRED_PROTOCOL" --quiet --show-progress --output-document $archive $downloadUrl
        ...
        if [ $? -eq "0" ]; then
            ...
            # Compatibility mode for older php.
            php5dismod ioncube
            php5enmod ioncube/01
        fi
    }
    ...
    function download_keyhelp()
    {
        ...
        local get_version="https://install.keyhelp.de/get_version.php?encode=plain&php_version=`get_php_version`$release_channel"

        declare -A release_info
        release_info[error]=true
        release_info[error_msg]='Invalid response. / Server not reachable.'

        # Contact server and load release infos.
        while read line
        do
...
#=======================================================================================================================
    ...
    printHeadline "Preparing KeyHelp installation, this may take a few minutes..."
    check_root
    check_supported_os
